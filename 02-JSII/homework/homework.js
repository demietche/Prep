...
for (var i = 1; i < palabras.length; i++) {frase = frase + ' ' + palabras[i]}
return frase
}
...
function numeroMasGrande(numeros) {
  ...
  // Tu código:
var max = numeros[0];
for (var i = 1; i < numeros.length; i++) {if(numeros[i] > max) {max = numeros[i];}}
return max
}
...
function cuentoElementos(arreglo){
  ...
  // Tu código:
var count = 0;
for (var i = 0; i < arreglo.length; i++) {if(arreglo[i] > 18) {count++;}}
return count
}
...
for (var i = 0; i < arreglo.length; i++) {if(arreglo[i] !== referencia) {return false;}} 
return true
} 
